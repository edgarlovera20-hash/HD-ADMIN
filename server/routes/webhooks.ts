...
const WORKFLOWS: Record<string, { humanReviewRequired: boolean }> = {
  ADMIN_FINANCE_ANOMALY_ALERT: { humanReviewRequired: true },
};
...
router.post("/", requireN8nSecret, (req, res) => {
  try {
    ...
    console.log(`[AUDIT STUB] n8n webhook workflowId=${workflowId} correlationId=${correlationId} platform=HD-ADMIN actorType=n8n_workflow severity=warning`);
